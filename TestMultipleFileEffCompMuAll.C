...

{
  ...
  TFile* infile        = new TFile(inpath+"/TauEfficiency_MuonAllID_mA19_OldTauID.root");
 ...
 if(TEfficiency::CheckConsistency(h_Num6,h_Den6))
  ...
  //Yaxis->Draw();
  leg->AddEntry(E,"#tau_{h} Loose Muon ID", "l");
  leg->AddEntry(E3,"#tau_{h} Medium Muon ID ", "l");
  leg->AddEntry(E5,"#tau_{h} Tight Muon ID ", "l");
  leg->AddEntry(E2,"#tau_{h} MuonCleaned Loose Muon ID ", "l");
  leg->AddEntry(E4,"#tau_{h} MuonCleaned Medium Muon ID ", "l");
  leg->AddEntry(E6,"#tau_{h} MuonCleaned Tight Muon ID ", "l");
  ...
  if(Num=="NumdR")
    {
      ...
      canvas->SaveAs(outpath+"/"+Name+"mA19_Muon_AllID_Comparison_OldTauID.png");
  ...
  else if(Num=="NumTotaldR")
    {
      ...
      std::string Name=Num.substr(pos);
      std::cout << "  " << Name <<std::endl;
      canvas->SaveAs(outpath+"/"+Name+"mA19_MuonAllID_Comparison_OldTauID.png");
...
void TestMultipleFileEffCompMuAll()
