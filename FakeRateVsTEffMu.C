...
std::vector<double> JetEffVector(string Num,string Denom,string inputfile)
{
  ...
  TString Dir12="JetFakeTauTight";
  TString Dir13 ="JetFakeTauVTight";
  TString Dir14 ="JetFakeTauVVTight";
...
void FakeRateVsTEffMu(){
  using namespace std;

  string File1="TauEfficiencyMu_ma20_NewTauID.root";
  ...
  VLooseG.SetMarkerColor(kGreen);
  VLooseG.SetLineColor(kGreen);
  gPad->Update();
  leg->AddEntry(&VVLooseG,"UnCleaned #tau_{h} New Tau ID Working points ", "l");    
  leg->AddEntry(&VLooseG,"MuonCleaned #tau_{had} New Tau ID Working points ", "l");
  ...
  gPad->Update();                                                                                                                  
  gPad->RedrawAxis();
  canvas->SaveAs(outpath+"/"+"JetFakeRateVSTauEffMu_NewTauID_mA20_Log.png");
