...

{
  ...
  TString outpath = "/home/redwan/PlotTestScripts/JetFakeTau";
  
    
  TFile* infile        = new TFile(inpath+"/JetFakeTau_Loose_ElectronID_Loose_MuonID.root");                                                        
  //TFile* infile2       = new TFile(inpath+"/test_TauEffMCleaned.root");                                                       


  TString Dir ="JetFake";
  TString Dir2 ="JetFakeE";
  TString Dir3 ="JetFakeM";
  ...
  //legend                                                                                                                                                                                                                                                                       
  TLegend *leg = new TLegend(0.50,0.55,0.80,0.85);
  ...
  TEfficiency *E3   =(TEfficiency*)infile->Get(Dir3+"/"+histName)->Clone("E3");
  ...
  leg->AddEntry(E2,"#tau_{h}electron cleaned", "l");
  leg->AddEntry(E3,"#tau_{h}muon cleaned", "l");
  ...
  graph2->SetMinimum(0);
  graph2->SetMaximum(0.4);
  gPad->Update();
  gPad->RedrawAxis();
  //Yaxis->Draw();
  auto graph3 = E3->GetPaintedGraph();
  graph3->SetMinimum(0);
  graph3->SetMaximum(0.4);
  gPad->Update();
  gPad->RedrawAxis();

  
  
  canvas->SaveAs(outpath+"/"+histName+"JetFakeTauLoose_E_Loose_M.png");
...

{
  ...
  Parameter.push_back("JetTauFake");
