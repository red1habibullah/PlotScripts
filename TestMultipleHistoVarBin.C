//plot Multiple histograms from multiple root files with variable bin length and customized tag name  
#include "TString.h"
#include "TH1F.h"
#include "TLatex.h"
#include "TAttLine.h"
...

{
  ...
  //Creating a variable tag for the canvas
  std::string tag = std::to_string(a);

  //canvas name has to be conveted to variable string                                                                                                                     
  ...
  VarbinFlag=((histName == "chargedIsoPtSum") ||(histName == "neutralIsoPtSum") || (histName == "photonPtSumOutsideSignalCone"));
  ...
  if(VarbinFlag)
    {
      ...
      Double_t bins[11]={0.0,0.1,0.25,0.5,0.75,10,15,20,30,40,50};
      TH1F *hnew = (TH1F*)h->Rebin(10,"hnew",bins);
      hnew->Draw("hist");
     
      
      TH1F *h2new = (TH1F*)h2->Rebin(10,"h2new",bins);
      h2new->Draw("hist same");
      
      
      
      TH1F *h3new = (TH1F*)h3->Rebin(10,"h3new",bins);
  ...
  canvas->SaveAs(outpath+"/"+histName+"CompareCutVarlog.png");
