//MultipleHistograms+Normalized to unity+Several times for different variables+ From the same file but different directories+ The Font sizes are fixed here
#include "TString.h"
#include "TH1F.h"
#include "TLatex.h"
#include "TAttLine.h"
...

{
  ...
  //TFile* infile        = new TFile(inpath+"/test_JetCompare.root");//The RootFile with the histogram
  ...
  //TFile* infile        = new TFile(inpath+"/test_validate_NoIso.root");
  //TFile* infile        = new TFile(inpath+"/test_validate_MuonIso_Boosted.root");
  TFile* infile        = new TFile(inpath+"/test_LowBoost.root"); 
  //TFile* infile        = new TFile(inpath+"/test_validate_RepfCand.root");
  //TFile* infile        = new TFile(inpath+"/test_validate_MuonIso_Map.root");
  // TString Dir1 ="CompareJet";
  // TString Dir2 ="CompareJetECleaned";
  // TString Dir3 ="CompareJetMCleaned";

  TString Dir1 ="ValidateTau";
  TString Dir2 ="ValidateTauE";
  TString Dir3 ="ValidateTauM";
  //TString Dir4 ="ValidateTauB";
  ...
  //get histograms
  //add legend entries                                                                                                                                                                                                                                       
  TH1F *h    = (TH1F*)infile->Get(Dir1+"/"+histName)->Clone("h");
  TH1F *h2   =(TH1F*)infile->Get(Dir2+"/"+histName)->Clone("h2");
  TH1F *h3   =(TH1F*)infile->Get(Dir3+"/"+histName)->Clone("h3");
  //TH1F *h4   =(TH1F*)infile->Get(Dir4+"/"+histName)->Clone("h4");

  bool RebinFlag =  false;
  //bool VarbinFlag = false;
  //bool LogXFlag= true;

  //LogXFlag=(histName == "JetEnergy"); 
  ...
  // h4->Draw("hist same");
  // Double_t scale_h4 =1/h4->Integral();
  // h4->Scale(scale_h4);
  // h4->SetLineWidth(2);
  ...
  */
  leg->AddEntry(h,"slimmedTaus ", "l");
  leg->AddEntry(h2,"slimmedTausElectronCleaned", "l");
  leg->AddEntry(h3,"slimmedTausMuonCleaned", "l");
  //leg->AddEntry(h4,"slimmedTausBoosted", "l");
  ...
  canvas->SaveAs(outpath+"/"+histName+"CompareTauLowBoostIso.png");
...
void TestSingleFileMultipleDirectory()

{
  vector <string> Parameter;
  
  Parameter.push_back("decayMode");
  ...
  // Parameter.push_back("neutralHadronEnergyFraction");
   // Parameter.push_back("neutralHadronEnergy");

   // Parameter.push_back("chargedHadronEnergyFraction");
   // Parameter.push_back("chargedHadronEnergy");
   // Parameter.push_back("photonEnergy");
      
   // //Parameter.push_back("#phi vs #eta");
   // Parameter.push_back("neutralHadronMultiplicity");
   // Parameter.push_back("chargedHadronMultiplicity");
   // Parameter.push_back("electronMultiplicity");
   // Parameter.push_back("muonMultiplicity");
   // Parameter.push_back("chargedMultiplicity");
   // Parameter.push_back("neutralMultiplicity");
   // Parameter.push_back("photonMultiplicity");
   // Parameter.push_back("photonEnergyFraction");
   // Parameter.push_back("JetEnergy");
