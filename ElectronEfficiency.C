...

{
  ...
  //input/output paths                                                                                                                  
  TString inpath =  "/home/redwan/PlotTestScripts/RootFiles/ANFiles";
  TString outpath = "/home/redwan/PlotTestScripts/PlotsDModeVsGenVars/MuoEle";

  TFile* infile        = new TFile(inpath+"/ElectronEfficiency_mA20.root");                                                        
  

  TString Dir ="EleEffLoose";
  TString Dir2 ="EleEffMedium";
  TString Dir3 ="EleEffTight";
  ...
  //legend                                                                                                                                                                                                                                                                       
  TLegend *leg = new TLegend(0.60,0.25,0.80,0.45);
  ...
  //RebinFlag=(histName == "TauEff");
	...
	E->SetTitle("DecayMode Finding Efficiency for Decay Mode 10 -> 3 Prong; Jet Pt(GeV);#epsilon");
      ...
      {
        E->Draw("AP");
        E->SetTitle("Electron (Reco+ID) Efficiency; Gen #tau_{e} Pt (GeV);#epsilon");
      }
	else if(ID)
      {
        E->Draw("AP");
        E->SetTitle("Electron ID Efficiency; Gen #tau_{e} Pt (GeV);#epsilon");
      }
  ...
  //Yaxis->Draw();
  leg->AddEntry(E,"Loose Electron ID", "l");
  leg->AddEntry(E2,"Medium ELectron ID", "l");
  leg->AddEntry(E3,"Tight Electron ID", "l");

  //mg->Draw();
  leg->Draw();
  title->DrawTextNDC(0.13,0.91,"CMS");
  extra->DrawTextNDC(0.23,0.91,"Simulation");
  lumi.DrawLatexNDC(0.9,0.91,"2017 Style Simulation");
  gPad->Update();
  ...
  else if(Num=="NumElectron" )
    {
      std::size_t pos = Num.find("E");
      std::string Name=Num.substr(pos);
      std::cout << "  " << Name <<std::endl;
      canvas->SaveAs(outpath+"/"+Name+"mA20_ID_Reco_Efficiency.png");

    }
    else if(Num=="NumElecID" )
    {
      std::size_t pos = Num.find("E");
      std::string Name=Num.substr(pos);
      std::cout << "  " << Name <<std::endl;
      canvas->SaveAs(outpath+"/"+Name+"mA20_ID_Efficiency.png");
  ...
  
{
  ...
  string DenomElectron="DenomElectron";
  string NumElectron="NumElectron";
  string DenomElecID="DenomElecID";
  string NumElecID="NumElecID";                                                                                                  
  ...
  HistSingleVal(NumElectron,DenomElectron,1);
  HistSingleVal(NumElecID,DenomElecID,2);
